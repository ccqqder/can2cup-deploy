...
| LINE 官方帳號:每月訊息額度(這台用 `wrangler.toml` 裡的 `PUSH_BUDGET`、`PUSH_USER_BUDGET` 自己限制推播量) | LINE 通知停止,直到下個月 |

## 出過的狀況

| 時間(台北) | 大家看到什麼 | 原因 | 修正 |
|---|---|---|---|
| 2026-09-05 到 09-07 | LINE 通知沒送到 | LINE 官方帳號的每月訊息額度用完了,而被拒的推播沒有留下紀錄 | can2cup 0.12.2 起每一則被拒的推播都會記錄 |
