...
| LINE 官方账号:每月消息额度(这台用 `wrangler.toml` 里的 `PUSH_BUDGET`、`PUSH_USER_BUDGET` 自行限制推送量) | LINE 通知停止,直到下个月 |

## 出过的状况

| 时间(台北) | 大家看到什么 | 原因 | 修复 |
|---|---|---|---|
| 2026-09-05 到 09-07 | LINE 通知没送达 | LINE 官方账号的每月消息额度用完了,而被拒绝的推送没有留下记录 | can2cup 0.12.2 起每一条被拒绝的推送都会记录 |
